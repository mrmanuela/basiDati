...
void dump_result_set(MYSQL* conn, MYSQL_STMT* stmt, char* title)
{
	...
	do {
			...
			while (true) {
				...
				for (i = 0; i < num_fields; i++) {
					...
					case MYSQL_TYPE_TIMESTAMP:
						...
						printf(" %02d-%02d-%d %02d:%02d:%02d.%-*d |", date->day, date->month, date->year, date->hour, date->minute, date->second, (int)fields[i].max_length - 20, (date->second_part) / 1000);
						break;

					case MYSQL_TYPE_DATE:
						date = (MYSQL_TIME*)rs_bind[i].buffer;
						printf(" %02d-%02d-%-*d |", date->day, date->month, (int)fields[i].max_length, date->year);
						break;

					case MYSQL_TYPE_TIME:
						date = (MYSQL_TIME*)rs_bind[i].buffer;
						printf(" %02d:%02d:%02d.%-*d |", date->hour, date->minute, date->second, (int)fields[i].max_length - 9, (date->second_part) / 1000);
						break;
					...
					case MYSQL_TYPE_LONG:
					case MYSQL_TYPE_SHORT:
						printf(" %-*d |", (int)fields[i].max_length + 3, *(int*)rs_bind[i].buffer);
						break;
					case MYSQL_TYPE_TINY:
						printf(" %-*hhd |", (int)fields[i].max_length + 2, *(int*)rs_bind[i].buffer);
						break;
		...
		if (status > 0)
			finish_with_stmt_error(conn, stmt, "An error occurred when fetching a new result\n", true);
	} while (status == 0);
}